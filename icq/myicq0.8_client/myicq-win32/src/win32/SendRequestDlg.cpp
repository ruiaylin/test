...
: CMyDlg(CSendRequestDlg::IDD, pParent), IcqWindow(WIN_SEND_REQUEST, c->uin)
{
...
: CMyDlg(CSendRequestDlg::IDD, pParent), IcqWindow(WIN_SEND_REQUEST, msg->uin)
{	
	...
	contact = icqLink->findContact(uin);
	m_text = text.c_str();
...
BOOL CSendRequestDlg::OnInitDialog() 
{
	...
	if (p->type == ICQ_PLUGIN_NET)
		icon = (HICON) ((NetPlugin *) p)->getNameIcon(name);
	else if (p->type == ICQ_PLUGIN_EXE) {
		icon = (HICON) ((ExePlugin *) p)->icon;
		name = ((ExePlugin *) p)->locName;
	}
	...
	int i = getApp()->getImageIndex(contact->face);
	m_faceButton.SetIcon(getApp()->largeImageList.ExtractIcon(i));
...
void CSendRequestDlg::OnTimer(UINT nIDEvent) 
{
	if (nIDEvent == IDT_ANIM) {
		frame ^= 1;
		int i = getApp()->getImageIndex(contact->face, frame ? STATUS_AWAY : STATUS_ONLINE);
		m_faceButton.SetIcon(getApp()->largeImageList.ExtractIcon(i));
	} else
