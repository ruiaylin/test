...
inline bool isAdmin(QID &qid)
{
	return (qid.uin == ADMIN_UIN && qid.domain.empty());
}
...
{
	out << type << flags << qid << when << text;
	if (type == MSG_TEXT)
		out << format.fontName << format.fontSize << format.fontColor << format.flags;
}

void IcqMsg::load(DBInStream &in)
{
	in >> type >> flags >> qid >> when >> text;
	if (type == MSG_TEXT)
		in >> format.fontName >> format.fontSize >> format.fontColor >> format.flags;
}

void IcqMsg::encode(TextOutStream &out)
{
	out << text.c_str();

	if (type == MSG_TEXT) {
		out << format.fontName.c_str() << format.fontSize;
		out << format.fontColor << format.flags;
	}
}

void IcqMsg::decode(const char *t)
{
	if (type == MSG_TEXT) {
		TextInStream in(t);
		in >> text;
		in >> format.fontName >> format.fontSize;
		in >> format.fontColor >> format.flags;
	} else
		text = t;
}
