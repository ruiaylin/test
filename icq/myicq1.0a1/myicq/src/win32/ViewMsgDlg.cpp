...
CViewMsgDlg::CViewMsgDlg(IcqContact *c, CWnd* pParent /*=NULL*/)
: CMyDlg(CViewMsgDlg::IDD, pParent), IcqWindow(WIN_VIEW_MESSAGE, &c->qid)
{
...
{
	CSendMsgDlg *dlg;
	IcqWindow *win = icqLink->findWindow(WIN_SEND_MESSAGE, &qid);
	if (!win) {
		dlg = new CSendMsgDlg(contact);

		CRect rc;
		GetWindowRect(rc);
		dlg->SetWindowPos(NULL, rc.left, rc.top, 0, 0, SWP_NOSIZE | SWP_NOZORDER);
...
{
	IcqMsg *msg = ((CIcqDlg *) AfxGetMainWnd())->fetchMsg(&qid);
	...
	GetDlgItem(IDC_NEXT_MSG)->EnableWindow(icqLink->findPendingMsg(&qid) != NULL);
}
...
{
	if (qid.uin)
		CViewDetailDlg *win = new CViewDetailDlg(qid, this);
}
