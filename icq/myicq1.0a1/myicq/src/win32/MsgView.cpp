...
void CMsgView::showMsg(IcqMsg *msg)
{
	if (msg) {
		...
		if (msg->type == MSG_TEXT) {
			...
			CString text;
			getMsgText(msg, text);
			m_msgEdit.SetWindowText(text);
		}

		if (sendFace >= 0) {
			m_sendFace.SetIcon(getApp()->getLargeFace(sendFace));
			GetDlgItem(IDC_FACE_SENDER)->ShowWindow(SW_SHOW);
		} else
			GetDlgItem(IDC_FACE_SENDER)->ShowWindow(SW_HIDE);

		if (recvFace >= 0) {
			m_recvFace.SetIcon(getApp()->getLargeFace(recvFace));
