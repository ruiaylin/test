...
CNotifyWnd::CNotifyWnd(HICON icon, const char *text, int t)
{
	if (notifyWnd)
		delete notifyWnd;
	notifyWnd = this;
	...
	if (nIDEvent == IDT_MOVE) {
		...
		if (rc.top >= rcWorkArea.bottom - wndHeight)
			SetWindowPos(&wndTopMost,
				rc.left, rc.top, rc.Width(), rc.Height(), SWP_SHOWWINDOW | SWP_NOACTIVATE);
		else {
			KillTimer(nIDEvent);
			if (waitTime > 0)
				SetTimer(IDT_WAIT, waitTime * 1000, NULL);
		}
