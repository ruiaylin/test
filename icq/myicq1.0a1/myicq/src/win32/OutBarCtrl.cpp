...
void OutBarFolder::addItem(const char *text, HICON icon)
{
	OutBarItem *item = new OutBarItem(text, icon);
...
void OutBarCtrl::addItem(int folder, const char *text, HICON icon)
{
	if (folder >= 0 && folder < getFolderCount())
		folders[folder]->addItem(text, icon);
}
...
void OutBarCtrl::animateFolder(int from, int to)
{
	CClientDC dc(this);
	CDC memDC;
...
void OutBarCtrl::drawItemImage(CDC *pDC, int i, CRect &rc)
{
	...
	HICON icon = folders[selFolder]->items[i]->icon;
	DrawIconEx(*pDC, rc.left, rc.top, icon, w, h, 0, NULL, DI_NORMAL);
}
...
void OutBarCtrl::drawAll(CDC *pDC)
{
	CDC memDC;
 ...
 */
void OutBarCtrl::removeItem(int folder, int i)
{
	vector<OutBarItem *> &items = folders[folder]->items;
	delete items[i];
