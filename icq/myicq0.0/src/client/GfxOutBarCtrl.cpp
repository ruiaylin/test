		...
		if (::GetCapture() == NULL)
		{
			...
			for (;;)
			{
				...
				case WM_LBUTTONUP:
					{	
						MSG msg1;
						VERIFY(::GetMessage(&msg1,NULL,0,0));
						if(GetCapture()==this&&(msg1.message==WM_LBUTTONDBLCLK||msg1.message==WM_LBUTTONDOWN))
						{
							KillTimer(4);
							GetOwner()->SendMessage(WM_OUTBAR_NOTIFY, NM_OB_ITEMDCLICK, 0);	
						}
						else
						{
							if (bHigh)
...
int CGfxOutBarCtrl::CBarFolder::InsertItem(int index, const char * text, const int image, const DWORD exData)
{
	...
	CBarItem * pbf = new CBarItem(text, image*2+1, exData);
	ASSERT(pbf);
...
BOOL CGfxOutBarCtrl::ModifyItem(DWORD dwID, int image,LPCTSTR lpszText)
{
	if(image<0&&lpszText==NULL)return FALSE;
	for(int i=0;i<arFolder.GetSize();i++)
	{		
		CBarFolder* pbf=(CBarFolder*)arFolder.GetAt(i);
		if(pbf->ModifyItem(dwID,image,lpszText)) return TRUE;
	}
	return FALSE;
}

BOOL CGfxOutBarCtrl::CBarFolder::ModifyItem(DWORD dwID, int image, LPCTSTR lpszText)
{
	for(int i=0;i<arItems.GetSize();i++)
	{
		...
		if(pItem->dwData==dwID)
		{
			if(image>=0)
			{
				if(pItem->iImageIndex%2)
					pItem->iImageIndex=image*2+1;
				else
					pItem->iImageIndex=image*2;
			}
			if(lpszText!=NULL)
			{
				if(pItem->cItem!=NULL)
				...
				pItem->cItem=new char[lstrlen(lpszText)+1];
				lstrcpy(pItem->cItem,lpszText);
			}
			return TRUE;
		}
	}
