...
start() {
	...
	PID=`ps aux | grep -w $PS_NAME | grep "${DB_PORT}" | grep -v grep | awk '{print $2}'`
    if [ "$PID" != "" ];
    then
        for p in $PID
        do
            echo "start $PS_NAME ( pid =" $p ")"
        done
    fi
...
stop() {
    ...
    then
        for ps in $PID
        do
            echo "kill -SIGINT $PS_NAME ( pid =" $ps ")"
			kill -2 $ps
        done
    fi
}
...
term() {
    ...
    then
        for ps in $PID
        do
            echo "kill -SIGKILL $PS_NAME ( pid =" $ps ")"
			kill -9 $ps
        done
    fi
}
...
list() {
	PID=`ps aux | grep -w $PS_NAME | grep "${DB_PORT}" | grep -v grep | awk '{printf("%s,%s,%s,%s\n", $1, $2, $9, $10)}'`
	if [ "$PID" != "" ];
	then
		echo "list ${PS_NAME} $role"
