...
//verify		:
//license		: GPL 2.0
//declaration	: Long live C! Fuck C++! C++, C'est la merde! C++, please go to hell!
...
#define	PTR_2_NUM(_ptr_)			({	\
										...
										(u8)(__epoll_data__.u64);	\
									})
#define	NUM_2_PTR(num)				({	\
										epoll_data_t	__epoll_data__;	\
										__epoll_data__.u64 = num;	\
										__epoll_data__.ptr;	\
									})
...
#define	IS_BT(_value, min, max)		({	\
										...
										IS_GT(__is_bt_value__, min) && IS_LT(__is_bt_value__, max);	\
									})
#define	IS_BE(_value, min, max)		({	\
										typeof(_value) __is_be_value__ = (_value);	\
										IS_GE(__is_be_value__, min) && IS_LE(__is_be_value__, max);	\
									})
#define	IS_NBT(_value, min, max)	({	\
										typeof(_value) __is_nbt_value__ = (_value);	\
										IS_LE(__is_nbt_value__, min) || IS_GE(__is_nbt_value__, max);	\
									})
#define	IS_NBE(_value, min, max)	({	\
										typeof(_value) __is_nbe_value__ = (_value);	\
										IS_LT(__is_nbe_value__, min) || IS_GT(__is_nbe_value__, max);	\
									})
...
#define	PTR_OFFSET(ptr0, ptr1)		({	\
										...
										TERN_CMP(IS_LT, __offset_p0__, __offset_p1__,	\
...
#define	ALLOC(ptr, size)			({	ptr = (typeof(ptr))(malloc(size));	})
#define	REALLOC(ptr, size)			({	\
										vp buf = realloc(ptr, size);	\
										ptr = (buf != NULL) ? ((typeof(ptr))(buf)) : ptr;	\
										buf;	\
									})
...
#define	ARR_OFFSET(e, a)			((size_t)(PTR_OFFSET(e, a) / sizeof(a[0])))
#define	ARR_ELEM_PTR(t, a, o)		((t)(a) + o)							//@a array, @o offset, @t type
...
#define	CMP(dst, src, len)			({	\
										...
										if (IS_NEQ(__cmp_src__, __cmp_dst__) && IS_GT(__cmp_len__, 0)) {	\
											__cmp_flag__ = memcmp((void*)__cmp_dst__,	\
															(void*)__cmp_src__, __cmp_len__);	\
										}	\
										__cmp_flag__;	\
									})
...
#else
#define	offsetof(stt, mem)			(n4)((&((typeof(stt)*)0)->mem))
