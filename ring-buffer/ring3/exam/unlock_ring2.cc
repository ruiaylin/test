...
n4 main(n4 argc, n1** argv)
{
	...
	struct timeval	end;
	pthread_t		producer_trd[80];
	pthread_t		consumer_trd[1];
...
n4	pc_queue_init(vpp queue_pptr, u4 size)
{
	...
	num = SIZE_K(128);
	ret = ring_init(&(queue->queue), num, size);
	if (IS_NZR(ret))	{
		sem_destroy(queue->sem);
		RET_INT(-2, "ring_init(&queue = %p, size = %u) = %d",
...
n4	pc_queue_push(vp queue_ptr, vp data)
{
	...
	do {
		ret = ring_push(queue->queue, data);
		if (IS_ZR(ret))	{
			break;
		}

		// fprintf(stderr, "ring_push(queue = %p, data = %p) = %d",
...
n4	pc_queue_pop(vp queue_ptr, vp data)
{
	...
	ret = ring_pop(queue->queue, data);
	if (IS_NZR(ret))	{
		RET_INT(-2, "ring_pop(queue = %p, data = %p) = %d",
					queue->queue, data, ret);
	}
...
n4	pc_queue_size(vp queue_ptr, u4* size)
{
	...
	ret = ring_size(queue->queue, size);
	if (IS_NZR(ret))	{
		RET_INT(-2, "ring_size(queue = %p, &size = %p) = %d",
					queue->queue, size, ret);
	}
...
vp producer(vp arg)
{
	...
	idx = lisk_atom_read(g_cpu_idx);
	lisk_atom_add(g_cpu_idx, 1);
	set_cpu_affinity(idx);
	// pinfo("producer idx = %d", idx);
...
vp consumer(vp arg)
{
	...
	for (idx = 0; IS_LT(idx, top); idx++) {
		...
		if (IS_NZR(ret) || IS_NL(ptr)) {
			printf("queue.pop(idx = %d) = %d, ptr = %p\n",
					idx, ret, ptr);
			idx--;
			continue;
		}
